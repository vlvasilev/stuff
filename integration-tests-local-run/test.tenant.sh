...
# /home/hisshadow/.garden/cache/dev-virtual/seeds/alicloud/kubeconfig.yaml
seed_kubeconfig=$3
shoot_namespace=$4
...
do
  ...
  # kubectl --kubeconfig=$seed_kubeconfig -n $shoot_namespace get deploy kube-apiserver-logger
  # while [[ "$?" != "0" ]]
  # do
  #   echo "Deployment kube-apiserver-logger still exist"
  #   sleep 30
  #   kubectl --kubeconfig=$seed_kubeconfig -n $shoot_namespace get deploy kube-apiserver-logger
  # done

  # kubectl --kubeconfig=$seed_kubeconfig -n $shoot_namespace get deploy logger
  # while [[ "$?" != "0" ]]
  # do
  #   echo "Deployment logger still exist"
  #   sleep 30
  #   kubectl --kubeconfig=$seed_kubeconfig -n $shoot_namespace get deploy logger
  # done

  # wget 'http://localhost:3100/loki/api/v1/query' -O-  --header="X-Scope-OrgID: operator" --post-data="query=count_over_time({pod_name=~\"logger-.*\"}[24h])"

  # echo "Delete leftovers from other tests"
  # for ns in $(kubectl --kubeconfig=$kubeconfig -n garden get ns | grep shoot--test | awk '{print $1}'); do ((kubectl --kubeconfig=$kubeconfig -n $ns delete pod --all --force --grace-period=0) &) ; done
  # for ns in $(kubectl --kubeconfig=$kubeconfig -n garden get ns | grep shoot--test | awk '{print $1}'); do ((kubectl --kubeconfig=$kubeconfig delete namespace $ns --force --grace-period=0) &) ; done
  ...
  # kubectl --kubeconfig=$kubeconfig delete cluster --all --force --grace-period=0

  echo "start test"
  go test -timeout=0 /home/hisshadow/go/src/github.com/gardener/gardener/test/suites/shoot --v -ginkgo.v -ginkgo.progress -ginkgo.noColor --report-file=/tmp/report.json --disable-dump=false -kubecfg="/home/hisshadow/.garden/dev/garden-gke/kubeconfig" -shoot-name=$shoot_name -project-namespace=garden-i330716 -ginkgo.focus="should get container logs from loki by tenant" > "$shoot_name/test-$i/test-logs"
  ...
  #     kubectl --kubeconfig=$kubeconfig -n garden cp garden/$flb_pod:logs "$shoot_name/test-$i/$flb_pod/file_out-$flb_pod"
  ...
  # for ns in $(kubectl --kubeconfig=$kubeconfig -n garden get ns | grep shoot--test | awk '{print $1}'); do ((kubectl --kubeconfig=$kubeconfig delete namespace $ns --grace-period=0) &) ; done
  # kubectl --kubeconfig=$kubeconfig delete ns garden --grace-period=0
  # kubectl --kubeconfig=$kubeconfig delete cluster --all --grace-period=0
