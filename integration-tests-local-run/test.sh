...
do
  ...
  echo "start test"
  go test -timeout=0 /home/hisshadow/go/src/github.com/gardener/gardener/test/suites/shoot --v -ginkgo.v -ginkgo.progress -ginkgo.noColor --report-file=/tmp/report.json --disable-dump=false -kubecfg="/home/hisshadow/.garden/dev/garden-gke/kubeconfig" -shoot-name=$shoot_name -project-namespace=garden-i330716 -ginkgo.focus="should get container logs from loki for all namespaces" > "$shoot_name/test-$i/test-logs"
  
  grep "retry failed with context deadline exceeded, last error: received only" "$shoot_name/test-$i/test-logs" -IR
  if [[ "$?" == "0" ]]
  then
    echo "Test $1 failed!!!!!!!!!!!!!!!!!!!!!!!!!!!"
    ...
    for pod in $pods;
    do
      result=$(kubectl --kubeconfig=$kubeconfig -n garden exec loki-0 -c loki wget -- 'http://localhost:3100/loki/api/v1/query' -O- --post-data="query=count_over_time({pod_name=\"$pod\"}[2h])")
    ...
    kubectl --kubeconfig=$kubeconfig -n garden logs loki-0 -c loki > "$shoot_name/test-$i/loki-logs"
